...
_AUTO_FIGURE_COUNTER = 0


def set_all_seeds(seed):
    """Set all RNG seeds used by this script for reproducible model initialization/training."""
    random.seed(seed)
    np.random.seed(seed)
...
def set_active_output_folder(output_folder):
    """Route any unsaved plt.show() figures to the active run folder in batch mode."""
    global ACTIVE_OUTPUT_FOLDER
    ACTIVE_OUTPUT_FOLDER = output_folder


def _save_unsaved_figures_instead_of_show(*args, **kwargs):
    """Save unsaved figures instead of opening GUI windows."""
    global _AUTO_FIGURE_COUNTER
    if ACTIVE_OUTPUT_FOLDER:
        figures_dir = os.path.join(ACTIVE_OUTPUT_FOLDER, "figures")
        os.makedirs(figures_dir, exist_ok=True)
        for fig_num in plt.get_fignums():
            fig = plt.figure(fig_num)
            if getattr(fig, "_saved_by_script", False):
                continue
            _AUTO_FIGURE_COUNTER += 1
            filepath = os.path.join(figures_dir, f"auto_saved_plot_{_AUTO_FIGURE_COUNTER:04d}.png")
            fig.savefig(filepath, dpi=150, bbox_inches='tight')
            fig._saved_by_script = True
            print(f"Saved: {filepath}")
    plt.close('all')


plt.show = _save_unsaved_figures_instead_of_show
...
def create_output_folder(prefix="polgun_cnn"):
    ...
    """
    forced_folder = os.environ.get("POLGUN_OUTPUT_FOLDER")
    folder_name = forced_folder if forced_folder else f"{prefix}_{datetime.now().strftime('%Y-%m-%d_%H%M%S')}"
    figures_path = os.path.join(folder_name, "figures")
    os.makedirs(figures_path, exist_ok=True)
    set_active_output_folder(folder_name)
...
def save_epoch_history_report(output_folder, history):
    ...
    for i in range(n_epochs):
        rec = {
            ...
            'train_mcc': history.get('train_mcc', [None]*n_epochs)[i],
            'val_mcc': history.get('val_mcc', [None]*n_epochs)[i],
...
def save_test_metrics_report(output_folder, test_metrics, data, pos_weight):
    ...
    When ``test_metrics`` includes ``per_file_test_metrics`` (one bundle per
    source CSV in the test folder), the same per-file bundles are mirrored
    into ``test_metrics_by_file.csv`` so the multi-file test split can be
    ...
    report = {
        'test_metrics': public_test_metrics,
        'class_distribution': {
            'train': {
    ...
    per_file = test_metrics.get('per_file_test_metrics')
    if per_file:
        decision_threshold = test_metrics.get('decision_threshold')
        rows = []
        for fname in sorted(per_file.keys()):
            bundle = per_file[fname]
            real = bundle.get('real_distribution', {}) or {}
            pred = bundle.get('predicted_distribution', {}) or {}
            cm = bundle.get('confusion_matrix') or [[0, 0], [0, 0]]
            tn = int(cm[0][0]); fp = int(cm[0][1])
            fn = int(cm[1][0]); tp = int(cm[1][1])
            rows.append({
                'source_file': fname,
                'decision_threshold': bundle.get('decision_threshold', decision_threshold),
                'n_samples': bundle.get('n_samples'),
                'n_stable_0': int(real.get('0', 0)),
                'n_increase_1': int(real.get('1', 0)),
                'n_pred_0': int(pred.get('0', 0)),
                'n_pred_1': int(pred.get('1', 0)),
                'accuracy': bundle.get('test_accuracy'),
                'precision': bundle.get('precision'),
                'recall': bundle.get('recall'),
                'f0_5_score': bundle.get('f0_5_score'),
                'f1_score': bundle.get('f1_score'),
                'f2_0_score': bundle.get('f2_0_score'),
                'pr_auc': bundle.get('pr_auc'),
                'mcc': bundle.get('mcc'),
                'true_negative': tn,
                'false_positive': fp,
                'false_negative': fn,
        ...
        csv_path = os.path.join(output_folder, "test_metrics_by_file.csv")
        pd.DataFrame(rows).to_csv(csv_path, index=False)
        print(f"Saved: {csv_path}")


def save_test_probability_report(output_folder, test_metrics, threshold: float = 0.5):
    """
    Save per-sample test probabilities, a probability histogram, and
    confidence diagnostics that separate CORRECT vs WRONG test decisions.
    The decision ``threshold`` (default 0.5) is the validation-committed t*
    and drives the borderline window (+/- 0.05), the histogram's vertical
    line, and the threshold-aware confidence/margin calculations.

    Threshold-aware confidence (``normalized_decision_confidence``) maps the
    distance from the actual decision boundary into [0, 1]:
      - if predicted INCREASE (prob >= t): (prob - t) / (1 - t)
      - if predicted HOLD    (prob <  t): (t - prob) / t
    so a probability barely past t is low-confidence even when t != 0.5.

    Outputs written to ``output_folder``:
      - ``test_decision_probabilities.csv``         per-sample probabilities,
        correctness, confusion-type, and threshold-aware confidence/margin.
      - ``test_decision_confidence_summary.json``   counts and mean/median
        confidence by correct vs wrong and by TN/FP/FN/TP, plus a
        high-confidence-wrong tally.
      - ``figures/test_decision_probability_histogram.png``
      - ``figures/test_correct_vs_wrong_probability_histogram.png``
      - ``figures/test_correct_vs_wrong_margin_histogram.png``
      - ``figures/test_confusion_type_probability_histogram.png``
      - ``figures/test_confidence_by_outcome_boxplot.png``
    """
    probs = np.array(test_metrics.get("_test_probabilities", []), dtype=float)
    preds = np.array(test_metrics.get("_test_predictions", []), dtype=int)
    y_true = np.array(test_metrics.get("_test_true_labels", []), dtype=int)

    if len(probs) == 0:
        print("No test probabilities found; skipping probability report.")
        return
    ...
    decision_type = np.empty(len(probs), dtype=object)
    decision_type[(y_true == 0) & (preds == 0)] = "true_negative"
    decision_type[(y_true == 0) & (preds == 1)] = "false_positive"
    decision_type[(y_true == 1) & (preds == 0)] = "false_negative"
    decision_type[(y_true == 1) & (preds == 1)] = "true_positive"

    prob_df = pd.DataFrame({
        "sample_index": np.arange(len(probs)),
        "true_label": y_true,
        ...
        "margin_from_threshold": abs_margin,
        ...
        "is_borderline_narrow": abs_margin <= band_narrow,
        "is_borderline_wide": abs_margin <= band_wide,
    })

    csv_path = os.path.join(output_folder, "test_decision_probabilities.csv")
    prob_df.to_csv(csv_path, index=False)
    print(f"Saved: {csv_path}")

    fig, ax = plt.subplots(figsize=(10, 6))
    ax.hist(probs, bins=np.linspace(0, 1, 41), color="steelblue", edgecolor="black", alpha=0.8)
    ax.axvline(threshold, color="red", linestyle="--", linewidth=2,
               label=f"Decision threshold = {threshold:.4f}")
    ax.axvspan(max(0.0, threshold - band_narrow), min(1.0, threshold + band_narrow),
               color="orange", alpha=0.2,
               label=f"Borderline: +/- {band_narrow}")
    ax.set_title("Test Set Decision Probability Distribution")
    ax.set_xlabel("Predicted Probability of INCREASE")
    ax.set_ylabel("Number of Test Samples")
    ax.set_xlim(0, 1)
    ax.legend()
    ax.grid(True, alpha=0.3)
    plt.tight_layout()
    save_figure(fig, output_folder, "test_decision_probability_histogram")
    plt.close(fig)

    probs_correct = probs[is_correct]
    probs_wrong = probs[~is_correct]
    margin_correct = abs_margin[is_correct]
    margin_wrong = abs_margin[~is_correct]
    n_correct = int(is_correct.sum())
    n_wrong = int((~is_correct).sum())

    fig, ax = plt.subplots(figsize=(10, 6))
    bins_p = np.linspace(0, 1, 41)
    if n_correct > 0:
        ax.hist(probs_correct, bins=bins_p, color="seagreen", edgecolor="black",
                alpha=0.6, label=f"Correct (n={n_correct})")
    if n_wrong > 0:
        ax.hist(probs_wrong, bins=bins_p, color="crimson", edgecolor="black",
                alpha=0.6, label=f"Wrong (n={n_wrong})")
    ax.axvline(threshold, color="black", linestyle="--", linewidth=2,
               label=f"Decision threshold = {threshold:.4f}")
    ax.set_title("Test Probability: Correct vs Wrong Decisions")
    ax.set_xlabel("Predicted Probability of INCREASE")
    ax.set_ylabel("Number of Test Samples")
    ax.set_xlim(0, 1)
    ax.legend()
    ax.grid(True, alpha=0.3)
    plt.tight_layout()
    save_figure(fig, output_folder, "test_correct_vs_wrong_probability_histogram")
    plt.close(fig)

    max_margin = float(max(threshold, 1.0 - threshold))
    bins_m = np.linspace(0.0, max_margin if max_margin > 0 else 1.0, 41)
    fig, ax = plt.subplots(figsize=(10, 6))
    if n_correct > 0:
        ax.hist(margin_correct, bins=bins_m, color="seagreen", edgecolor="black",
                alpha=0.6, label=f"Correct (n={n_correct})")
    if n_wrong > 0:
        ax.hist(margin_wrong, bins=bins_m, color="crimson", edgecolor="black",
                alpha=0.6, label=f"Wrong (n={n_wrong})")
    ax.axvline(band_narrow, color="orange", linestyle=":", linewidth=1.5,
               label=f"Borderline margin = {band_narrow}")
    ax.set_title(f"Test Margin from Threshold (t={threshold:.4f}): Correct vs Wrong")
    ax.set_xlabel("|prob - threshold|  (larger = more confident decision)")
    ax.set_ylabel("Number of Test Samples")
    ax.set_xlim(0, bins_m[-1])
    ax.legend()
    ax.grid(True, alpha=0.3)
    plt.tight_layout()
    save_figure(fig, output_folder, "test_correct_vs_wrong_margin_histogram")
    plt.close(fig)

    type_colors = {
        "true_negative":  "steelblue",
        "false_positive": "crimson",
        "false_negative": "darkorange",
        "true_positive":  "seagreen",
    }
    type_order = ["true_negative", "false_positive", "false_negative", "true_positive"]
    fig, axes = plt.subplots(2, 2, figsize=(13, 9), sharex=True, sharey=False)
    for ax_t, dtype in zip(axes.flatten(), type_order):
        mask = (decision_type == dtype)
        n = int(mask.sum())
        if n > 0:
            ax_t.hist(probs[mask], bins=bins_p, color=type_colors[dtype],
                      edgecolor="black", alpha=0.75)
        ax_t.axvline(threshold, color="black", linestyle="--", linewidth=1.5,
                     label=f"t = {threshold:.4f}")
        ax_t.set_title(f"{dtype.replace('_', ' ').title()}  (n={n})")
        ax_t.set_xlabel("Predicted Probability of INCREASE")
        ax_t.set_ylabel("Count")
        ax_t.set_xlim(0, 1)
        ax_t.grid(True, alpha=0.3)
        ax_t.legend(loc="upper center", fontsize=8)
    fig.suptitle("Test Probability by Confusion Type", fontsize=14)
    plt.tight_layout()
    save_figure(fig, output_folder, "test_confusion_type_probability_histogram")
    plt.close(fig)
    ...
    cw_labels = []
    if n_correct > 0:
        cw_data.append(norm_conf[is_correct]); cw_labels.append(f"Correct\n(n={n_correct})")
    if n_wrong > 0:
        cw_data.append(norm_conf[~is_correct]); cw_labels.append(f"Wrong\n(n={n_wrong})")
    if cw_data:
        bp1 = ax_cw.boxplot(cw_data, labels=cw_labels, patch_artist=True, showmeans=True)
        for patch, color in zip(bp1['boxes'], ["seagreen", "crimson"][:len(cw_data)]):
            patch.set_facecolor(color); patch.set_alpha(0.6)
    ax_cw.set_title("Threshold-aware Confidence: Correct vs Wrong")
    ax_cw.set_ylabel("Normalized decision confidence  [0, 1]")
    ax_cw.set_ylim(-0.02, 1.02)
    ax_cw.grid(True, alpha=0.3, axis="y")
    ...
    for dtype in type_order:
        ...
        n = int(mask.sum())
        if n > 0:
            ct_data.append(norm_conf[mask])
            ct_labels.append(f"{dtype.split('_')[0][0].upper()}{dtype.split('_')[1][0].upper()}\n(n={n})")
            ct_colors.append(type_colors[dtype])
    if ct_data:
        bp2 = ax_ct.boxplot(ct_data, labels=ct_labels, patch_artist=True, showmeans=True)
        for patch, color in zip(bp2['boxes'], ct_colors):
            patch.set_facecolor(color); patch.set_alpha(0.6)
    ax_ct.set_title("Threshold-aware Confidence by Confusion Type")
    ax_ct.set_ylabel("Normalized decision confidence  [0, 1]")
    ax_ct.set_ylim(-0.02, 1.02)
    ax_ct.grid(True, alpha=0.3, axis="y")

    fig.suptitle(f"Test Confidence by Outcome  (t* = {threshold:.4f})", fontsize=13)
    plt.tight_layout()
    save_figure(fig, output_folder, "test_confidence_by_outcome_boxplot")
    plt.close(fig)
    ...
    def _group_stats(mask):
        ...
        if n == 0:
            return {
                "count": 0,
                "mean_prob_increase": None,
                "median_prob_increase": None,
                "mean_abs_margin": None,
                "median_abs_margin": None,
                "mean_normalized_confidence": None,
                "median_normalized_confidence": None,
            }
        return {
            "count": n,
            "mean_prob_increase": float(np.mean(probs[mask])),
            "median_prob_increase": float(np.median(probs[mask])),
            "mean_abs_margin": float(np.mean(abs_margin[mask])),
            "median_abs_margin": float(np.median(abs_margin[mask])),
            "mean_normalized_confidence": float(np.mean(norm_conf[mask])),
            "median_normalized_confidence": float(np.median(norm_conf[mask])),
        }

    high_conf_threshold = 0.8
    wrong_mask = ~is_correct
    high_conf_wrong_mask = wrong_mask & (norm_conf >= high_conf_threshold)
    high_conf_correct_mask = is_correct & (norm_conf >= high_conf_threshold)

    summary = {
        "decision_threshold": float(threshold),
        "n_total": int(len(probs)),
        "groups": {
            "correct": _group_stats(is_correct),
            "wrong":   _group_stats(wrong_mask),
            "true_negative":  _group_stats(decision_type == "true_negative"),
            "false_positive": _group_stats(decision_type == "false_positive"),
            "false_negative": _group_stats(decision_type == "false_negative"),
            "true_positive":  _group_stats(decision_type == "true_positive"),
        },
        "high_confidence_threshold": high_conf_threshold,
        "high_confidence_wrong": {
            "count": int(high_conf_wrong_mask.sum()),
            "fraction_of_wrong": (float(high_conf_wrong_mask.sum() / max(n_wrong, 1))
                                  if n_wrong > 0 else None),
            "fraction_of_all": float(high_conf_wrong_mask.sum() / len(probs)),
        },
        "high_confidence_correct": {
            "count": int(high_conf_correct_mask.sum()),
            "fraction_of_correct": (float(high_conf_correct_mask.sum() / max(n_correct, 1))
                                    if n_correct > 0 else None),
            "fraction_of_all": float(high_conf_correct_mask.sum() / len(probs)),
        },
        "notes": (
            "normalized_decision_confidence maps |prob - t| into [0, 1] using the "
            "actual decision threshold t (not 0.5), so confidence is comparable "
            "across models with different chosen thresholds."
        ),
    }

    summary_path = os.path.join(output_folder, "test_decision_confidence_summary.json")
    with open(summary_path, "w", encoding="utf-8") as f:
        json.dump(summary, f, indent=2)
    print(f"Saved: {summary_path}")


def compute_validation_pr_threshold(
    models,
    forward_fn,
    data,
    output_folder=None,
    criterion: str = "max_mcc",
    default_threshold: float = 0.5,
):
    """
    Run the trained model on validation, build the precision-recall curve, and
    score every candidate threshold with F1 and MCC. Pick a single decision
    threshold t* using the chosen criterion (max_mcc, max_f1, or fixed).

    Saves three artifacts in ``output_folder``:
      - ``val_pr_curve_points.csv``: row per candidate threshold with columns
        ``threshold, precision, recall, f1, mcc, predicted_positive_count``.
        Convention: row ``i`` uses sklearn's ``thresholds[i]`` paired with
        ``precision[i]`` and ``recall[i]``; the trivial ``(P=1, R=0)`` tail
        point is dropped because it has no associated threshold.
      - ``figures/val_pr_curve.png``: PR curve (left) and F1/MCC vs threshold
        (right), with markers at t=0.5, argmax F1, and argmax MCC.
      - ``val_threshold_selection.json``: AP, argmax-F1 row, argmax-MCC row,
        metrics at 0.5, and the committed ``chosen_threshold`` (with criterion).

    Returns ``(chosen_threshold, selection_info)`` so the caller can apply the
    same scalar to the test set without re-tuning.
    """
    ...
    if n_pos == 0 or n_neg == 0:
        print(f"Validation set has only one class (pos={n_pos}, neg={n_neg}); "
              f"PR curve / threshold tuning is undefined. Falling back to {default_threshold}.")
        return float(default_threshold), {}
    ...
    with warnings.catch_warnings():
        ...
        for i, t in enumerate(thresholds):
            ...
            mcc_arr[i] = matthews_corrcoef(y_true, y_pred) if y_pred.sum() not in (0, len(y_pred)) else 0.0
    ...
    idx_max_f1 = int(np.argmax(f1_arr))
    idx_max_mcc = int(np.argmax(mcc_arr))

    def _row_to_dict(idx):
        return {
    ...
    row_max_f1 = _row_to_dict(idx_max_f1)
    row_max_mcc = _row_to_dict(idx_max_mcc)
    ...
    metrics_at_default = {
        ...
        "mcc": float(matthews_corrcoef(y_true, y_pred_05)) if y_pred_05.sum() not in (0, len(y_pred_05)) else 0.0,
    ...
    if crit == "max_f1":
        ...
        print(f"Unknown threshold criterion '{criterion}'; falling back to max_mcc.")
        crit = "max_mcc"
        chosen = row_max_mcc
    ...
    selection_info = {
        ...
        "csv_path": os.path.join(output_folder, "val_pr_curve_points.csv") if output_folder else None,
        "note": (
            "Row i in val_pr_curve_points.csv uses sklearn thresholds[i] paired with "
            "precision[i] and recall[i]; the trivial (P=1, R=0) tail point with no threshold is dropped."
        ),
    }
    ...
    if output_folder is not None:
        ...
        ax_pr = axes[0]
        ax_pr.plot(recall, precision, color="steelblue", linewidth=2, label=f"PR curve (AP={average_precision:.4f})")
        ...
        ax_pr.scatter([metrics_at_default["recall"]], [metrics_at_default["precision"]],
                      ...
                      label=f"t=0.5 (F1={metrics_at_default['f1']:.3f}, MCC={metrics_at_default['mcc']:.3f})")
        ...
        ax_t = axes[1]
        ax_t.plot(thresholds, precision[:n_t], color="steelblue", linewidth=1.5, label="Precision", alpha=0.7)
        ax_t.plot(thresholds, recall[:n_t], color="green", linewidth=1.5, label="Recall", alpha=0.7)
        ...
        ax_t.axvline(default_threshold, color="black", linestyle=":", linewidth=1, label=f"t=0.5")
        ax_t.axvline(row_max_f1["threshold"], color="darkorange", linestyle="--", linewidth=1, alpha=0.7,
                     label=f"argmax F1 (t={row_max_f1['threshold']:.3f})")
        ax_t.axvline(row_max_mcc["threshold"], color="crimson", linestyle="--", linewidth=1, alpha=0.7,
                     label=f"argmax MCC (t={row_max_mcc['threshold']:.3f})")
        ax_t.set_xlabel("Threshold")
        ax_t.set_ylabel("Metric value")
        ax_t.set_xlim(0.0, 1.0)
        ax_t.set_ylim(min(-0.1, float(np.min(mcc_arr)) - 0.05), 1.02)
    ...
    print(f"  argmax F1:  t={row_max_f1['threshold']:.4f}  P={row_max_f1['precision']:.4f}  R={row_max_f1['recall']:.4f}  F1={row_max_f1['f1']:.4f}  MCC={row_max_f1['mcc']:.4f}")
    print(f"  argmax MCC: t={row_max_mcc['threshold']:.4f}  P={row_max_mcc['precision']:.4f}  R={row_max_mcc['recall']:.4f}  F1={row_max_mcc['f1']:.4f}  MCC={row_max_mcc['mcc']:.4f}")
    print(f"  t=0.5:      P={metrics_at_default['precision']:.4f}  R={metrics_at_default['recall']:.4f}  F1={metrics_at_default['f1']:.4f}  MCC={metrics_at_default['mcc']:.4f}")
...
def save_model_architecture_report(output_folder, models, hyperparams, model_type, data):
    ...
    report = {
        ...
        'hyperparameters': {
            ...
            'lr_decay_epoch': hyperparams.get('lr_decay_epoch'),
            'lr_decay_gamma': hyperparams.get('lr_decay_gamma'),
        },
    }
...
def get_system_specs():
    """
    Collects hardware and software specs for reproducibility reporting.
    Uses psutil/cpuinfo if installed, falls back gracefully otherwise.
    """
    specs = {
    ...
    try:
        import sklearn
        specs['sklearn_version'] = sklearn.__version__
    except Exception:
        pass
    ...
    try:
        ...
        specs['cpu_model'] = info.get('brand_raw', specs['cpu_model'])
        if specs['cpu_cores_physical'] is None:
            specs['cpu_cores_physical'] = info.get('count', None)
    except ImportError:
        pass
...
def save_system_and_timing(output_folder, specs, training_seconds, run_timestamp):
    """
    Saves system specs + training wall-clock time as both JSON and TXT.
    """
    mins, secs = divmod(int(training_seconds), 60)
    hrs, mins = divmod(mins, 60)
    if hrs > 0:
        formatted = f"{hrs}h {mins}m {secs}s"
    else:
        formatted = f"{mins}m {secs}s"

    report = {
        'system': specs,
        'training_wall_clock_seconds': round(training_seconds, 2),
        'training_wall_clock_formatted': formatted,
        'run_timestamp': run_timestamp,
    }

    json_path = os.path.join(output_folder, "system_and_timing.json")
    with open(json_path, "w", encoding="utf-8") as f:
        json.dump(report, f, indent=2)
    print(f"Saved: {json_path}")
    ...
    lines = []
    lines.append("=" * 60)
    lines.append("SYSTEM & TRAINING ENVIRONMENT")
    lines.append("=" * 60)
    lines.append(f"  CPU:              {specs['cpu_model']}")
    cores_str = str(specs.get('cpu_cores_physical') or '?')
    lines.append(f"  Cores:            {cores_str} physical / {specs['cpu_cores_logical']} logical")
    ram_str = f"{specs['ram_total_gb']} GB" if specs['ram_total_gb'] else "unavailable"
    lines.append(f"  RAM:              {ram_str}")
    lines.append(f"  OS:               {specs['os']}")
    lines.append(f"  Python:           {specs['python_version']}")
    lines.append(f"  PyTorch:          {specs['pytorch_version']}")
    lines.append(f"  NumPy:            {specs['numpy_version']}")
    lines.append(f"  pandas:           {specs['pandas_version']}")
    lines.append(f"  scikit-learn:     {specs.get('sklearn_version', 'N/A')}")
    lines.append("")
    lines.append("=" * 60)
    lines.append("TRAINING TIME")
    lines.append("=" * 60)
    lines.append(f"  Wall-clock time:  {formatted} ({training_seconds:.2f} seconds)")
    lines.append(f"  Run started:      {run_timestamp}")
    lines.append("=" * 60)

    txt_path = os.path.join(output_folder, "system_and_timing.txt")
...
# # ---------------------------------------------------------------
    ...
    # When scale_sensors=False the scaler is fit downstream (post train/val split)
    # and applied to the windowed sequences via _apply_scaler_to_sensor_channels;
    # NaN values are preserved here so they survive into the windowed array.
    if scale_sensors:
        if scaler is None:
            raise ValueError("scale_sensors=True requires a fitted scaler.")
...
def _apply_scaler_to_sensor_channels(X, scaler, missing_value=0.0):
    """Apply ``scaler`` to the first three sensor channels of ``X`` while
    preserving NaN positions as ``missing_value`` after scaling.

    Args:
        X: array of shape (N, seq_len, 4); only channels 0/1/2 are scaled.
        scaler: fitted StandardScaler with three input features.
        missing_value: value used to mark previously-NaN sensor samples.

    Returns: a copy of ``X`` with sensor channels standardised.
    """
    if X.size == 0:
        return X
    X = X.copy()
    sensor = X[:, :, [0, 1, 2]].astype(float, copy=True)
    nan_mask = np.isnan(sensor)
    sensor_filled = np.where(nan_mask, 0.0, sensor)
    flat = sensor_filled.reshape(-1, 3)
    flat_scaled = scaler.transform(flat)
    sensor_scaled = flat_scaled.reshape(sensor.shape)
    sensor_scaled[nan_mask] = missing_value
    X[:, :, [0, 1, 2]] = sensor_scaled
    return X
...
def prepare_all_data(train_folder, test_folder, sequence_length,
    ...
    Order of operations (no train -> val/test scaler leakage):
      1. Read CSV files from the train and test folders.
      2. Build sequences for every training file with raw, unscaled sensor
         values (NaN preserved).
      3. Shuffle-split the training sequences into train/val (85/15).
      4. Fit a NaN-aware StandardScaler ONLY on the training-portion
         sensor values (channels 0, 1, 2).
      5. Apply that scaler to the train, val, and test sequences and
         replace NaN entries with ``missing_value``.
    ...
    # --- BUILD UNSCALED TRAIN SEQUENCES (NaN preserved) ---
    print("Processing Train Sequences (unscaled, NaN preserved)...")
    ...
    print(f"Splitting into train/val with shuffled train_test_split (85/15, random_state={split_seed})...")
    ...
    # --- BUILD UNSCALED TEST SEQUENCES (NaN preserved) ---
    print("Processing Test Sequences (unscaled, NaN preserved)...")
    X_test_list, y_test_list = [], []
    test_source_files_list = []
    for df in test_dfs:
        x_s, y_s = process_single_df_to_sequences(
            df, scaler=None, sequence_length=sequence_length,
            noise_thresholds=noise_thresholds,
            filter_quiet_negatives=False,
            missing_value=missing_value,
            scale_sensors=False,
        )
        if len(x_s) > 0:
            ...
            # Tag every sequence emitted by this CSV with its source filename so
            # downstream per-file test metrics can group samples by file.
            src_name = df.attrs.get('source_file', 'unknown_file')
            test_source_files_list.extend([src_name] * len(x_s))

    if X_test_list:
        X_test_raw = np.concatenate(X_test_list)
        y_test = np.concatenate(y_test_list)
    else:
        X_test_raw = np.empty((0, sequence_length, 4))
        y_test = np.empty((0, 1))

    test_source_files = np.array(test_source_files_list, dtype=object)

    # --- FIT SCALER ON TRAIN SPLIT ONLY (NaN-aware) ---
    print("Fitting NaN-aware Scaler on TRAIN split sensor values only...")
    train_sensor = X_train_raw[:, :, [0, 1, 2]].reshape(-1, 3) if X_train_raw.size else np.empty((0, 3))
    scaler = StandardScaler()
    if train_sensor.size == 0:
        scaler.mean_ = np.zeros(3)
        scaler.scale_ = np.ones(3)
        scaler.var_ = np.ones(3)
        scaler.n_samples_seen_ = np.zeros(3, dtype=int)
        print("  WARNING: training split is empty; using identity scaler.")
    else:
        scaler.mean_ = np.nanmean(train_sensor, axis=0)
        scaler.scale_ = np.nanstd(train_sensor, axis=0, ddof=0)
        # Guard against degenerate (zero-variance / all-NaN) channels
        scaler.scale_ = np.where(
            (~np.isfinite(scaler.scale_)) | (scaler.scale_ == 0),
            1.0,
            scaler.scale_,
        )
        scaler.var_ = scaler.scale_ ** 2
        scaler.n_samples_seen_ = np.sum(~np.isnan(train_sensor), axis=0)
    scaler.n_features_in_ = 3

    print(f"  Scaler mean (TRAIN-only, ignoring NaN): {scaler.mean_}")
    print(f"  Scaler std  (TRAIN-only, ignoring NaN): {scaler.scale_}")

    # --- APPLY SCALER TO TRAIN/VAL/TEST SEQUENCES ---
    X_train = _apply_scaler_to_sensor_channels(X_train_raw, scaler, missing_value=missing_value)
    X_val = _apply_scaler_to_sensor_channels(X_val_raw, scaler, missing_value=missing_value)
    X_test = _apply_scaler_to_sensor_channels(X_test_raw, scaler, missing_value=missing_value)
    ...
    data = {
        ...
        # Per-sample source CSV filename for the test set; aligns 1:1 with
        # X_test/y_test rows so per-file metrics can be computed without
        # re-running model inference per file.
...
def _extract_cnn_features(models, x):
    """Run the convolutional feature extractor and return flattened features.

    This is the single source of truth for the CNN feature path. Both
    ``forward_cnn`` (inference) and ``create_cnn_model`` (sizing the linear
    head via a dummy probe) call into it, so the linear input dimension is
    always whatever this function actually produces -- no hand-derived
    formulas to keep in sync if you uncomment a pool/conv3 line below.
    """
...
def _probe_cnn_feature_dim(feature_layers, input_dim, seq_len):
    """Return the flattened feature count produced by ``_extract_cnn_features``
    for a dummy input of shape ``(2, seq_len, input_dim)``.

    Layers are temporarily switched to ``eval()`` so BatchNorm does not
    complain about probe-batch statistics; original training/eval modes are
    restored before returning. The forward is wrapped in ``torch.no_grad()``
    so no gradient state or running-stats updates leak from the probe.
    """
    saved_modes = {}
    for name, layer in feature_layers.items():
        if hasattr(layer, 'training'):
            saved_modes[name] = layer.training
        if hasattr(layer, 'eval'):
            layer.eval()
    try:
        with torch.no_grad():
            dummy = torch.zeros(2, seq_len, input_dim)
            flat = _extract_cnn_features(feature_layers, dummy)
        return int(flat.shape[1])
    finally:
        for name, was_training in saved_modes.items():
            layer = feature_layers[name]
...
def create_cnn_model(
    ...
    conv2 = nn.Conv1d(n_filters1, n_filters1, kernel_size=kernel_size, padding=kernel_size // 2)
    bn2 = nn.BatchNorm1d(n_filters2)

    # Pooling: Shrinks time dimension by half (Downsampling). Always
    # constructed so it can be enabled inside _extract_cnn_features without
    # touching this function.
    pool = nn.MaxPool1d(kernel_size=pool_size, stride=pool_size)

    # NOTE: if you enable Block 3 inside _extract_cnn_features, also add
    # 'conv3'/'bn3' to feature_layers and the returned dict so the probe and
    # the optimizer see them. Example (kept commented to match the disabled
    # block in the helper):
    conv3 = nn.Conv1d(n_filters2, n_filters2, kernel_size=kernel_size, padding=kernel_size // 2)
    ...
    # Determine the linear input size by probing _extract_cnn_features so any
    # change to the conv/pool path is automatically reflected in the linear
    # head without re-deriving any length formula by hand.
    linear_input_size = _probe_cnn_feature_dim(feature_layers, input_dim, seq_len)
...
def forward_cnn(models, x):
    return models['linear'](_extract_cnn_features(models, x))
...
def train_model(
   ...
   # --- MODEL SWITCHING LOGIC RESTORED ---
    ...
    #lr_scheduler = optim.lr_scheduler.MultiStepLR(
     #   optimizer,
      #  milestones=[5,10,15,20,25,30,35,40,45,50],#milestones=[max(0, lr_decay_epoch - 1)]
    ...
    def _safe_mcc(y_true_arr, y_pred_arr):
        s = int(np.asarray(y_pred_arr).sum())
        if s == 0 or s == len(y_pred_arr):
            return 0.0
        with warnings.catch_warnings():
            warnings.simplefilter("ignore")
            return float(matthews_corrcoef(y_true_arr, y_pred_arr))
    ...
    for epoch in range(epochs):
        ...
        train_mcc = _safe_mcc(all_train_labels, all_train_preds)
        ...
        with torch.inference_mode():
            ...
            val_mcc = _safe_mcc(val_labels, val_preds_binary)
    ...
    print(f">>> Restored best weights from epoch {best_epoch+1} (val_loss={best_val_loss:.6f}).")
    ...
    print(f">>> Training summary: best_epoch={best_epoch+1}, best_val_loss={best_val_loss:.6f}, "
          f"last_epoch_run={len(history['train_loss'])}, stopped_early={epochs_without_improvement >= early_stopping_patience}")
...
def analyze_activation_statistics(
    ...
    with torch.inference_mode():
        ...
        for ch_idx in range(n_channels):
            ...
            heatmap_pos_per_filter_channel[:, ch_idx] = (
                pos.detach().cpu().mean(dim=(0, 2)).numpy()
            )
            heatmap_neg_per_filter_channel[:, ch_idx] = (
...
def _compute_classification_metrics(probs, y_true, threshold, include_pr_auc=True):
    """Compute the standard test metric bundle from probabilities and labels.

    Shared by the overall test evaluation and the per-file test evaluation so
    every reported metric uses identical formulas and the same decision
    ``threshold``. PR-AUC is suppressed when only one class is present, which
    happens often on small per-file slices.
    """
    ...
    pr_auc = None
    if include_pr_auc and len(np.unique(y_true)) > 1:
        pr_auc = float(average_precision_score(y_true, probs))

    if preds.sum() in (0, n) or y_true.sum() in (0, n):
        mcc = 0.0
    else:
    ...
    confidence = np.maximum(probs, 1.0 - probs)
    probability_summary = {
        'mean_prob_increase': float(np.mean(probs)),
        'median_prob_increase': float(np.median(probs)),
        'std_prob_increase': float(np.std(probs, ddof=1)) if n > 1 else 0.0,
        'mean_confidence': float(np.mean(confidence)),
        'median_confidence': float(np.median(confidence)),
        'borderline_45_55_count': int(np.sum((probs >= 0.45) & (probs <= 0.55))),
        'borderline_45_55_fraction': float(np.mean((probs >= 0.45) & (probs <= 0.55))),
        'borderline_40_60_count': int(np.sum((probs >= 0.40) & (probs <= 0.60))),
        'borderline_40_60_fraction': float(np.mean((probs >= 0.40) & (probs <= 0.60))),
        'confident_0_10_or_90_100_count': int(np.sum((probs <= 0.10) | (probs >= 0.90))),
        'confident_0_10_or_90_100_fraction': float(np.mean((probs <= 0.10) | (probs >= 0.90))),
    }
...
def _compute_per_file_test_metrics(probs, preds, y_true, source_files, threshold):
    """Group test-set predictions by their source CSV and compute one metric
    bundle per file at the validation-committed ``threshold``.

    Returns a dict keyed by filename. Files with no surviving sequences are
    silently skipped; PR-AUC and MCC are reported as ``None``/``0.0`` for
    single-class files (handled inside ``_compute_classification_metrics``).
    """
    probs = np.asarray(probs, dtype=float).flatten()
    y_true = np.asarray(y_true, dtype=int).flatten()
    source_files = np.asarray(source_files, dtype=object).flatten()

    if len(source_files) != len(probs):
        print(
            f"WARNING: per-file metrics skipped -- source_files length "
            f"({len(source_files)}) != probabilities length ({len(probs)})."
        )
        return {}

    per_file = {}
    unique_files = sorted({str(f) for f in source_files})
    for fname in unique_files:
        mask = (source_files == fname)
        bundle = _compute_classification_metrics(
            probs[mask], y_true[mask], threshold, include_pr_auc=True
        )
        if bundle is None:
            continue
        per_file[fname] = bundle
    return per_file


def evaluate_test_set(models, forward_fn, data, pos_weight, threshold: float = 0.5):
    """
    Evaluates the trained model on the test set at the given decision
    ``threshold`` (default 0.5). When a validation-derived t* is passed in,
    all reported metrics (precision, recall, F1, MCC, confusion matrix, etc.)
    are computed at that single committed threshold; the threshold itself is
    stored under ``decision_threshold`` for traceability.

    When ``data['test_source_files']`` is present, the returned dict also
    includes ``per_file_test_metrics`` keyed by source CSV filename, using the
    same threshold and metric helper as the overall report.
    """
    print("\n" + "="*30)
    ...
    overall = _compute_classification_metrics(probs, y_true, threshold, include_pr_auc=True)
    preds = (probs >= threshold).astype(int)
    ...
    print(f"F0.5 Score:    {overall['f0_5_score']:.4f}")
    print(f"F1 Score:      {overall['f1_score']:.4f}")
    print(f"F2.0 Score:    {overall['f2_0_score']:.4f}")
    pr_auc_disp = (f"{overall['pr_auc']:.4f}" if overall['pr_auc'] is not None else "n/a (single-class)")
    print(f"PR-AUC:        {pr_auc_disp}")
    print(f"MCC:           {overall['mcc']:.4f}")
    print(f"Mean confidence: {overall['probability_summary']['mean_confidence']:.4f}")
    print(f"Borderline 0.45-0.55: {overall['probability_summary']['borderline_45_55_count']} "
          f"({100*overall['probability_summary']['borderline_45_55_fraction']:.2f}%)")
    print(f"Borderline 0.40-0.60: {overall['probability_summary']['borderline_40_60_count']} "
          f"({100*overall['probability_summary']['borderline_40_60_fraction']:.2f}%)")
    print("-" * 30)
    print(f"Real Distribution:      {overall['real_distribution']}")
    print(f"Predicted Distribution: {overall['predicted_distribution']}")
    print("-" * 30)
    ...
    metrics = {
        'test_loss': test_loss,
        'test_accuracy': overall['test_accuracy'],
        'decision_threshold': float(threshold),
        'precision': overall['precision'],
        'recall': overall['recall'],
        'f0_5_score': overall['f0_5_score'],
        'f1_score': overall['f1_score'],
        'f2_0_score': overall['f2_0_score'],
        'pr_auc': overall['pr_auc'],
        'mcc': overall['mcc'],
        'probability_summary': overall['probability_summary'],
        'confusion_matrix': overall['confusion_matrix'],
        'real_distribution': overall['real_distribution'],
        'predicted_distribution': overall['predicted_distribution'],
        '_test_probabilities': probs.tolist(),
        '_test_predictions': preds.tolist(),
        '_test_true_labels': y_true.tolist(),
    }

    source_files = data.get('test_source_files')
    if source_files is not None and len(source_files) > 0:
        per_file = _compute_per_file_test_metrics(
            probs, preds, y_true, source_files, threshold
        )
        if per_file:
            metrics['per_file_test_metrics'] = per_file
            metrics['_test_source_files'] = [str(f) for f in source_files]
            n_files = len(per_file)
            print(f"Computed per-file test metrics for {n_files} source file(s) "
                  f"at decision threshold {threshold:.6f}.")
...
def plot_f05_curve(history, output_folder=None):
    ...
    if 'train_f05' not in history or len(history['train_f05']) == 0:
        return
    epochs = range(1, len(history['train_f05']) + 1)
    fig = plt.figure(figsize=(8, 5))
...
def plot_f2_curve(history, output_folder=None):
    ...
    if 'train_f2' not in history or len(history['train_f2']) == 0:
        return
    epochs = range(1, len(history['train_f2']) + 1)
    fig = plt.figure(figsize=(8, 5))
...
def plot_mcc_curve(history, output_folder=None):
    """Standalone MCC (train/val) vs epochs plot. MCC range is [-1, 1]."""
    if 'train_mcc' not in history or len(history['train_mcc']) == 0:
        return
    epochs = range(1, len(history['train_mcc']) + 1)
    fig = plt.figure(figsize=(8, 5))
    plt.plot(epochs, history['train_mcc'], label='Train MCC')
    plt.plot(epochs, history['val_mcc'], label='Val MCC', linestyle='--')
    plt.axhline(y=0.0, color='grey', linestyle=':', linewidth=1,
                label='MCC = 0 (random)')
    plt.title('Matthews Correlation Coefficient vs Epochs')
    plt.xlabel('Epochs')
    plt.ylabel('MCC')
    plt.ylim([-1.05, 1.05])
    plt.legend()
...
def save_training_summary_txt(output_folder, history, test_metrics, models, hyperparams, model_type, data):
    ...
    for i in range(n_epochs):
        ...
        row = (f"{i+1:5d} | {_g('train_loss',i):8.4f} | {_g('val_loss',i):8.4f} | "
               ...
               f"{_g('train_mcc',i):7.4f} | {_g('val_mcc',i):7.4f}")
        lines.append(row)
...
def test_single_file_simulation_with_ablation(filepath, models, forward_fn, scaler, sequence_length,
    ...
    print(f"Ablation Sim Config -> Step: {step_size:.2f} | Limit: {max_limit_A} | Ablation: {ablation_name}")
...
def test_single_file_simulation_with_single_ablation(filepath, models, forward_fn, scaler, sequence_length,
    ...
    print(f"Single Ablation Sim Config -> Step: {step_size:.2f} | Limit: {max_limit_A} | Ablation: {ablation_name}")
...
def _get_test_metric_value(report, key):
    """Read one scalar test metric from a saved per-run test_metrics.json report."""
    ...
    value = metrics.get(metric_key_map[key])
    return float(value) if value is not None else np.nan


def _read_val_chosen_threshold(run_folder):
    """Return the validation-committed t* for one run, or NaN if unavailable."""
    ...
    return (float(t) if t is not None else np.nan, crit)


def aggregate_multi_seed_results(root_output_folder, run_records):
    """Save individual run metrics and overall mean/std summaries for the full seed grid.

    Test metrics (accuracy, precision, recall, F1, PR-AUC, MCC) come from each
    run's ``test_metrics.json`` at its validation-committed decision threshold,
    which is included in the per-run CSV and in the aggregate summary so the
    threshold itself is reported alongside the metrics it generated.

    The validation threshold is reported as the mean (and SD) of the per-run
    ``chosen_threshold`` read from each run's ``val_threshold_selection.json``,
    so the multi-seed summary reflects the average t* across all completed
    runs rather than any single seed.
    """
    metric_names = ["accuracy", "precision", "recall", "f1", "pr_auc", "mcc",
                    "decision_threshold"]
    rows = []

    for record in run_records:
        metrics_path = os.path.join(record["output_folder"], "test_metrics.json")
        val_t, val_crit = _read_val_chosen_threshold(record["output_folder"])
        row = {
            "run_index": record["run_index"],
            "weight_init_seed": record["weight_init_seed"],
            "split_seed": record["split_seed"],
            "output_folder": record["output_folder"],
            "status": "missing_metrics",
            "val_chosen_threshold": val_t,
            "val_threshold_criterion": val_crit,
        }
        ...
        if os.path.exists(metrics_path):
            ...
            for metric_name in metric_names:
                row[metric_name] = _get_test_metric_value(report, metric_name)
        else:
            for metric_name in metric_names:
    ...
    all_runs_path = os.path.join(root_output_folder, "all_runs_metrics.csv")
    all_runs_df.to_csv(all_runs_path, index=False)
    print(f"Saved: {all_runs_path}")

    ok_df = all_runs_df[all_runs_df["status"] == "ok"]
    summary_rows = []
    for metric_name in metric_names:
        values = pd.to_numeric(ok_df[metric_name], errors="coerce").dropna()
        summary_rows.append({
            "metric": metric_name,
    ...
    val_threshold_values = pd.to_numeric(
        all_runs_df["val_chosen_threshold"], errors="coerce"
    ).dropna()
    val_threshold_summary = {
        "metric": "val_chosen_threshold",
        "n_runs": int(val_threshold_values.shape[0]),
        "mean": float(val_threshold_values.mean()) if len(val_threshold_values) else np.nan,
        "sd":   float(val_threshold_values.std(ddof=1)) if len(val_threshold_values) > 1 else 0.0,
    }
    summary_rows.append(val_threshold_summary)

    criteria_used = sorted({c for c in all_runs_df.get("val_threshold_criterion", [])
                            if isinstance(c, str) and c})

    summary_csv_path = os.path.join(root_output_folder, "aggregate_metrics_mean_sd.csv")
    summary_json_path = os.path.join(root_output_folder, "aggregate_metrics_mean_sd.json")
    pd.DataFrame(summary_rows).to_csv(summary_csv_path, index=False)
    with open(summary_json_path, "w", encoding="utf-8") as f:
        json.dump({
            "n_expected_runs": len(run_records),
            "n_completed_runs": int((all_runs_df["status"] == "ok").sum()),
            "metrics": summary_rows,
            "val_threshold_criteria_used": criteria_used,
            "notes": (
                "decision_threshold is the per-run test threshold (validation-committed t*) "
                "as stored in each run's test_metrics.json. "
                "val_chosen_threshold is the same t* read from each run's "
                "val_threshold_selection.json; the reported mean is the average across "
                "all completed runs."
            ),
        }, f, indent=2)
    ...
    for row in summary_rows:
        if row["metric"] == "val_chosen_threshold":
            continue
        print(f"  {row['metric']}: {row['mean']:.4f} ± {row['sd']:.4f} (n={row['n_runs']})")
    print(
        f"\nValidation chosen_threshold across runs: "
        f"{val_threshold_summary['mean']:.4f} ± {val_threshold_summary['sd']:.4f} "
        f"(n={val_threshold_summary['n_runs']})"
    )
    if criteria_used:
        print(f"Validation threshold criteria observed: {criteria_used}")


def run_multi_seed_experiments():
    """Launch 25 isolated single-run processes and aggregate the saved test metrics."""
    weight_init_seeds = [33, 545, 97, 123, 999]
    split_seeds = [12, 97, 435, 8765, 88]

    root_output_folder = os.path.abspath(create_output_folder(prefix="polgun_cnn_multiseed"))
    run_records = []
    ...
    print(f"Root output:       {root_output_folder}")
    print("=" * 80 + "\n")
    ...
    for weight_seed in weight_init_seeds:
        for split_seed in split_seeds:
            run_index += 1
            run_name = f"run_{run_index:02d}_init_{weight_seed}_split_{split_seed}"
            run_output_folder = os.path.join(root_output_folder, run_name)
            os.makedirs(os.path.join(run_output_folder, "figures"), exist_ok=True)

            print("\n" + "=" * 80)
            print(f"RUN {run_index}/{total_runs}: weight_init_seed={weight_seed}, split_seed={split_seed}")
            print(f"Output folder: {run_output_folder}")
            print("=" * 80)

            env = os.environ.copy()
            env["POLGUN_SINGLE_RUN"] = "1"
            env["POLGUN_WEIGHT_INIT_SEED"] = str(weight_seed)
            env["POLGUN_SPLIT_SEED"] = str(split_seed)
            env["POLGUN_OUTPUT_FOLDER"] = run_output_folder
    ...
    aggregate_multi_seed_results(root_output_folder, run_records)
...
if __name__ == "__main__":
    ...
    print(f"\n>>> Single-run seeds: weight_init_seed={WEIGHT_INIT_SEED}, split_seed={SPLIT_SEED}")
    ...
    HYPERPARAMS = {
        ...
        'missing_value': 0,              # Placeholder for missing/dropped data
    ...
    models, fwd_fn, history, initial_weights, final_weights, weight_history = train_model(
        ...
        lr_decay_epoch=HYPERPARAMS['lr_decay_epoch'],
        lr_decay_gamma=HYPERPARAMS['lr_decay_gamma'],
    )

    training_seconds = time.time() - train_start_time
    timing_report = save_system_and_timing(OUTPUT_FOLDER, system_specs, training_seconds, run_timestamp)
    ...
    # ==========================================
    # 6.2 Validation PR Curve + Threshold Selection (sets t* for test)
    # ==========================================
    ...
    # ==========================================
    test_metrics = evaluate_test_set(models, fwd_fn, data, pos_weight, threshold=chosen_threshold)
    if test_metrics:
        test_metrics['threshold_selection'] = threshold_selection
    ...
    if test_metrics:
        ...
        save_test_probability_report(OUTPUT_FOLDER, test_metrics, threshold=chosen_threshold)
...
#     )
    ...
    print(f"  - {OUTPUT_FOLDER}/test_metrics.json        (F0.5, F1, F2 + class distributions + per-file metrics)")
    print(f"  - {OUTPUT_FOLDER}/test_metrics_by_file.csv (per-test-file metrics at validation t*)")
    print(f"  - {OUTPUT_FOLDER}/model_architecture.json  (layer table + hyperparameters)")
    print(f"  - {OUTPUT_FOLDER}/system_and_timing.json   (hardware specs + training time)")
    print(f"  - {OUTPUT_FOLDER}/system_and_timing.txt    (human-readable specs)")
    print(f"\nCheckpoints:")
    ...
    print(f"  Training time:  {timing_report['training_wall_clock_formatted']} ({training_seconds:.1f}s)")
    print("="*70)
