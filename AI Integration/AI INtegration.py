...
FORCE_INCREASE_FIRST_N = 0  # Force decision=1 for the first N inference steps
                             # Set to 0 to disable forced override

USE_CSV_HISTORY_ONLY = True  # False: hybrid (CSV bootstrap + decision_log) [default]
                              # True : always use CSV VoltageChange for Prev_A1 history

SIMULATE_STREAMING = True     # True: stream-style rolling buffers with snapshot copy
                              # False: existing batch-style slicing from full arrays
...
def run_simulation(models, forward_fn, scaler, ckpt, df, time_col):
    ...
    # Sensor / label arrays for the whole file (raw, unscaled)
    sensor_raw = df[SENSOR_COLS].values                  # (N, 3)
    gt_vc      = df['VoltageChange'].values              # (N,)  ground truth

    # CSV Prev_A1 source (used directly in streaming mode; optional in batch mode)
    ...
    if SIMULATE_STREAMING:
        print("  Mode: STREAMING (rolling buffer + snapshot copy)")
        print("  Prev_A1 source: CSV (streaming buffer)")
    else:
        print("  Mode: BATCH (array slicing)")
    print("=" * 70)

    sensor_buf = None
    prev_a1_buf = None
    if SIMULATE_STREAMING:
        sensor_buf = deque(maxlen=seq_len)
        prev_a1_buf = deque(maxlen=seq_len)
        print("\n  Warming up streaming buffers...")
        for i in range(seq_len):
            sensor_buf.append(sensor_raw[i].copy())
            prev_a1_buf.append(float(csv_prev_a1[i]))
            print(f"    Warm-up tick {i+1}/{seq_len} buffered")
    ...
    for step in range(n_steps):
        ...
        # ============ T5 : Data extraction / snapshot copy (MEASURED) ============
        t5_start  = time.perf_counter()

        if SIMULATE_STREAMING:
            # Strict causal behaviour: snapshot FIRST, append current row AFTER.
            # Window contains only past rows [row_idx - seq_len .. row_idx - 1],
            # matching the batch-mode slice sensor_raw[win_start:win_end].
            sensor_window = np.array(sensor_buf, dtype=float, copy=True)
            prev_a1_window = np.array(prev_a1_buf, dtype=float, copy=True)
            t5_src = "Streaming snapshot (CSV Prev_A1, causal)"
        else:
            ...
            # Toggle with USE_CSV_HISTORY_ONLY:
            #   True  -> always use CSV VoltageChange
            #   False -> hybrid: CSV bootstrap, then decision_log
            prev_a1_window = np.zeros(seq_len, dtype=float)
            for k in range(seq_len):
                abs_row = win_start + k
                if USE_CSV_HISTORY_ONLY:
            ...
            t5_src = "CSV only" if USE_CSV_HISTORY_ONLY else "CSV + decision log"

        t5_ms = (time.perf_counter() - t5_start) * 1000.0
        all_t5.append(t5_ms)
        print(f"  T5  – Data extraction / snapshot ({t5_src}) ........ "
        ...
        # ============ Append current stream sample AFTER inference ============
        # This keeps the window strictly causal: row_idx is only visible to
        # the NEXT inference step, never to the one that just predicted it.
        if SIMULATE_STREAMING:
            sensor_buf.append(sensor_raw[row_idx].copy())
            prev_a1_buf.append(float(csv_prev_a1[row_idx]))
...
def main():
    ...
    print(f"Force-increase first N: {FORCE_INCREASE_FIRST_N}")
    print(f"Simulate streaming: {SIMULATE_STREAMING}")
    if SIMULATE_STREAMING:
        print("Prev_A1 source: CSV (streaming buffer)")
    else:
        print(f"Prev_A1 source: {'CSV ONLY' if USE_CSV_HISTORY_ONLY else 'HYBRID (CSV bootstrap + decision_log)'}")
