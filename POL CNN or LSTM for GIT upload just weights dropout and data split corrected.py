...
def prepare_all_data(train_folder, test_folder, sequence_length,
    ...
    NEW: Intra-file 85/15 train/val split (first 85% of each file for train, last 15% for val).
    NEW: NaN-aware normalization (ignores NaN when computing mean/std).
    
    Reads csv files from train and test folders,
    Splits EACH FILE into train (first 85%) and validation (last 15%) portions,
    Fits scaler on TRAIN portions only (NaN-aware),
    ...
    Outputs: data dict (X_train, y_train, X_val, y_val, X_test, y_test),
             train_files, val_files (file lists for verification)
    ...
    all_train_files = sorted(glob.glob(os.path.join(train_folder, "*.csv")))
    all_dfs = [pd.read_csv(f) for f in all_train_files]
    test_dfs = read_folder_csvs(test_folder)
    
    print(f"Loaded {len(all_train_files)} training files")
    
    # --- INTRA-FILE SPLIT ---
    # Split each file: first 85% for training, last 15% for validation
    train_dfs = []
    val_dfs = []
    
    for f, df in zip(all_train_files, all_dfs):
        n_rows = len(df)
        split_idx = int(train_val_split * n_rows)
        
        train_portion = df.iloc[:split_idx].copy()
        val_portion = df.iloc[split_idx:].copy()
        
        # Add source file info for debugging
        train_portion.attrs['source_file'] = os.path.basename(f) + "_train"
        val_portion.attrs['source_file'] = os.path.basename(f) + "_val"
        
        train_dfs.append(train_portion)
        val_dfs.append(val_portion)
    
    total_train_rows = sum(len(df) for df in train_dfs)
    total_val_rows = sum(len(df) for df in val_dfs)
    print(f"Intra-file split: {total_train_rows} train rows, {total_val_rows} val rows "
          f"({100*total_train_rows/(total_train_rows+total_val_rows):.1f}% / "
          f"{100*total_val_rows/(total_train_rows+total_val_rows):.1f}%)")
    
    # --- DEBUG: Print per-file split details ---
    print("\n--- PER-FILE SPLIT DETAILS ---")
    for f, df in zip(all_train_files, all_dfs):
        n_rows = len(df)
        split_idx = int(train_val_split * n_rows)
        print(f"  {os.path.basename(f)}: {n_rows} rows -> Train[0:{split_idx}] Val[{split_idx}:{n_rows}]")
    print("--- END SPLIT DETAILS ---\n")
    
    # --- NaN-AWARE SCALER ---
    # Fit scaler on TRAIN portions only, ignoring NaN values
    print("Fitting NaN-aware Scaler on TRAIN portions...")
    feature_cols = ["GunCurrent.Avg","peg-BL-cc:pressureM","RadiationTotal"]
    ...
    if X_train_list:
        ...
        X_train = np.empty((0, sequence_length, 4))
    ...
    for df in val_dfs:
        x_s, y_s = process_single_df_to_sequences(
            df, scaler, sequence_length,
            noise_thresholds=noise_thresholds,
            filter_quiet_negatives=False  # Don't filter val data
        )
        if len(x_s) > 0:
    ...
    if X_val_list:
        ...
        X_val = np.empty((0, sequence_length, 4))
        y_val = np.empty((0, 1))
    ...
    print("-" * 50)
    print(f"Intra-file Split: First {train_val_split*100:.0f}% train / Last {(1-train_val_split)*100:.0f}% val per file")
    print("-" * 50)
    print(f"CLASS DISTRIBUTION:")
    print(f"  TRAIN:      0={int(n_neg):>6}  |  1={int(n_pos):>6}  |  1-ratio={100*n_pos/(n_pos+n_neg):.2f}%")
    print(f"  VALIDATION: 0={int(n_neg_val):>6}  |  1={int(n_pos_val):>6}  |  1-ratio={100*n_pos_val/(n_pos_val+n_neg_val+1e-8):.2f}%")
    print(f"  TEST:       0={int(n_neg_test):>6}  |  1={int(n_pos_test):>6}  |  1-ratio={100*n_pos_test/(n_pos_test+n_neg_test+1e-8):.2f}%")
    print("-" * 50)
    print(f"Calculated Pos_Weight (for Train): {pos_weight.item():.4f}")
    print("="*50 + "\n")
    
    # --- OVERLAP CHECK ---
    # Check if any train samples are identical to val samples
    print("Checking for train/val overlap...")
    n_train = len(X_train)
    n_val = len(X_val)
    overlap_count = 0
    
    # Convert to hashable for comparison (check first 100 val samples for speed)
    check_limit = min(100, n_val)
    for i in range(check_limit):
        val_sample = X_val[i].tobytes()
        for j in range(n_train):
            if X_train[j].tobytes() == val_sample:
                overlap_count += 1
                break
    
    if overlap_count > 0:
        print(f"  WARNING: Found {overlap_count}/{check_limit} val samples that match train samples!")
    else:
        print(f"  OK: No overlap found in first {check_limit} val samples.")
    print("")
    
    # Return file lists for verification (all files used for both train and val)
    return data, pos_weight, scaler, all_train_files, all_train_files
...
def create_validation_ablations(X_val, y_val, missing_value=-100.0):
    ...
    - 'clean': Original validation data
    - 'current_full': Current channel fully missing
    - 'pressure_full': Pressure channel fully missing
    - 'radiation_full': Radiation channel fully missing
    - 'current_block': Current channel 50% block missing (middle)
    - 'pressure_block': Pressure channel 50% block missing (middle)
    - 'radiation_block': Radiation channel 50% block missing (middle)
    """
    seq_len = X_val.shape[1]
    block_start = seq_len // 4
    block_end = 3 * seq_len // 4  # 50% middle block
    
    ablations = {'clean': (X_val.clone(), y_val.clone())}
    ...
    for ch_idx, name in enumerate(sensor_names):
        ...
        ablations[f'{name}_full'] = (X_full, y_val.clone())
        
        # Block ablation (middle 50%)
        X_block = X_val.clone()
        X_block[:, block_start:block_end, ch_idx] = missing_value
        ablations[f'{name}_block'] = (X_block, y_val.clone())
...
def save_robustness_report(robustness_results, test_results, output_folder, epoch=None):
    """
    Saves formatted robustness metrics AND test results to a text file.
    """
    ...
    with open(filepath, 'w') as f:
        # ==================== ROBUSTNESS METRICS ====================
        f.write("=" * 90 + "\n")
        f.write("ROBUSTNESS VALIDATION REPORT\n")
        f.write("=" * 90 + "\n\n")
        ...
        if full_metrics:
            ...
            f.write(f"Full-Missing Avg:  Acc={avg_acc:.4f}  Prec={avg_prec:.4f}  Rec={avg_rec:.4f}\n")
        ...
        if block_metrics:
            ...
            f.write(f"Block-Missing Avg: Acc={avg_acc:.4f}  Prec={avg_prec:.4f}  Rec={avg_rec:.4f}\n")
        ...
        if test_results:
            ...
            f.write("\n" + "-" * 40 + "\n")
            if total_tests > 0:
                f.write(f"TOTAL: {passed_tests}/{total_tests} tests passed ")
                f.write(f"({100*passed_tests/total_tests:.1f}%)\n")
...
def test_file_split(train_files, val_files):
    """Verify intra-file 85/15 split is being used (same files for both)."""
    results = {}
    
    # For intra-file split, train_files and val_files should be the same list
    results['same_files_used'] = set(train_files) == set(val_files)
    results['files_exist'] = len(train_files) > 0
...
def test_ablation_suite(ablations, X_val, missing_value=-100.0):
    """Verify ablation variants are correct."""
    results = {}
    seq_len = X_val.shape[1]
    
    results['clean_matches_original'] = torch.allclose(ablations['clean'][0], X_val)
...
def run_validation_tests(train_files, val_files, X_train, X_val, 
                         models, forward_fn, ablations, robustness_results,
                         model_type='cnn', missing_value=-100.0):
    """Run all validation tests and return consolidated results."""
    all_results = {}
    
    all_results['file_split'] = test_file_split(train_files, val_files)
...
if __name__ == "__main__":
    ...
    HYPERPARAMS = {
        ...
        'train_val_split': 0.85,           # File-level split ratio
    ...
    # 2. Prepare Data (file-level split, NaN-aware normalization)
    data, pos_weight, scaler, train_files, val_files = prepare_all_data(
    ...
    # ==========================================
    # 3.5 ROBUSTNESS VALIDATION SUITE
    # ==========================================
    print("\n>>> Running Robustness Validation Suite...")
    
    # Create deterministic ablations of validation data
    ablations = create_validation_ablations(
        data['X_val'], data['y_val'], 
        missing_value=HYPERPARAMS['missing_value']
    )
    
    # Evaluate model on all ablation variants
    robustness_results = evaluate_robustness(models, fwd_fn, ablations)
    
    # Run validation test cases
    test_results = run_validation_tests(
        train_files, val_files,
        data['X_train'], data['X_val'],
        models, fwd_fn, ablations, robustness_results,
        model_type=MODEL_TYPE,
        missing_value=HYPERPARAMS['missing_value']
    )
    
    # Save comprehensive robustness report
    save_robustness_report(robustness_results, test_results, OUTPUT_FOLDER)
